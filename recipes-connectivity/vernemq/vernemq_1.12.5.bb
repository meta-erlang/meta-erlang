...
inherit rebar3 useradd update-rc.d systemd
...
do_compile_prepend() {
    ...
    install -D ${WORKDIR}/build_deps.sh.eleveldb ${S}/_build/default/lib/eleveldb/c_src/build_deps.sh
    install -D ${WORKDIR}/Makefile.c_src.eleveldb ${S}/_build/default/lib/eleveldb/c_src/Makefile
    install -D ${WORKDIR}/rebar.config.eleveldb ${S}/_build/default/lib/eleveldb/rebar.config

    # fix dependency: bcrypt
    install -D ${WORKDIR}/Makefile.c_src.bcrypt ${S}/_build/default/lib/bcrypt/c_src/Makefile
...
USERADD_PACKAGES = "${PN}"
USERADD_PARAM_${PN} = "--system --create-home --home-dir ${localstatedir}/lib/vernemq \
    --shell /bin/false --user-group vernemq"

SYSTEMD_SERVICE_${PN} = "vernemq.service"
