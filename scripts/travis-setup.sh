...
YOCTO_VERSION=$(echo $POKY | gawk -F "-" '{print $2}')
LAYERDIR=$(dirname $0)/..
SRC_DIR=${1-$(pwd)}
...
mkdir -p $SRC_DIR
cd $SRC_DIR
git clone --depth=1 --single-branch --branch=${POKY:-master} git://git.yoctoproject.org/poky
...
echo ". $SRC_DIR/poky/oe-init-build-env $SRC_DIR/build" >>$INIT
# This only works correctly in bash, hence /bin/bash.
. poky/oe-init-build-env $SRC_DIR/build
...
# Place meta-erlang as last BBLAYERS
sed -i -e "s;\(  \"\);  $SRC_DIR  \1;" conf/bblayers.conf
...
# ISP not correctly resolving www.example.com
CONNECTIVITY_CHECK_URIS ?= "https://www.google.com/"
EOF

# Use Amazon S3 bucket as sstate cache if available.
if [ -n "$AWS_BUCKET" ]; then
    echo "SSTATE_MIRRORS = \"file://.*  http://${AWS_BUCKET}.website.s3for.me/PATH\"" >>conf/local.conf
else
