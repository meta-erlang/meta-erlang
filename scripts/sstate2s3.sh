...
use_https = False
host_base = rest.s3for.me
host_bucket = %(bucket)s.rest.s3for.me
EOF
    fi
    ...
    s3cmd $S3CMD_DRYRUN --acl-public --no-progress --skip-existing --exclude-from=$S3EXCLUDE $S3_STORAGE_CLASS sync $SSTATE_CACHE/ s3://${AWS_BUCKET}/
else
